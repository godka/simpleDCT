	...
	public class Matrix 
	{
			...
			for (int i = 0; i < height; i++) {
				for (int j = 0; j < width; j++) {
					...
					if (i == 0) {
						...
						ci = Math.Sqrt ((double)2 / (double)width);
					}
		...
		public Matrix Normalize(){
			...
			for (int i = 0;i < elements.Length;i++) {
				data [i] = (elements [i] - min) / (max - min);
			}
			return new Matrix (numColumns, numRows, data);
		}
		/**
		 * 矩阵的转置
		 * 
		 ...
		 * 
		 * @param dblB - 一维数组，长度为矩阵的阶数，传入对称三对角蟮闹鞫越窍咴?兀禾
		 *			     返回时存放全部特征值。
		 ...
		 * 
		 * @param dblEigenValue - 一维数组，长度为矩阵的阶数，返回时存放特征值
		 * @param mtxEigenVector - 返回时存放特征蛄烤卣螅?渲械趇列为与数组
		 *                         dblEigenValue中第j个特征值对应的特征向量
		 * @param eps - 计算精度
		 * @return bool型，求解是否成功
		 */
